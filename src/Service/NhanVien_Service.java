/*
 * Click nbfs://nbhost/SystemFileSystem/Templates/Licenses/license-default.txt to change this license
 * Click nbfs://nbhost/SystemFileSystem/Templates/Classes/Class.java to edit this template
 */
package Service;

import Interface.InF_NhanVIen;
import Model.NhanVien;
import java.sql.Connection;
import java.sql.PreparedStatement;
import java.sql.ResultSet;
import java.util.ArrayList;
import java.util.List;
import java.sql.*;

/**
 *
 * @author Admin
 */
public class NhanVien_Service implements InF_NhanVIen {

    Connection con = null;
    PreparedStatement ps = null;
    ResultSet rs = null;
    String sql = null;

    public List<NhanVien> getAll_NV() {
        sql = "select MaNV,HoTen,GioiTinh,DiaChi,DienThoai,Email,Luong,ChucVu,TrangThai from tblnhanvien ";
        List<NhanVien> list = new ArrayList<>();
        try {
            con = DBconnect1.getConnection();
            ps = con.prepareStatement(sql);
            rs = ps.executeQuery();
            while (rs.next()) {
                NhanVien nv = new NhanVien(rs.getString(1), rs.getString(2),
                        rs.getString(3), rs.getString(4), rs.getInt(5),
                        rs.getString(6), rs.getInt(7), rs.getString(8),
                        rs.getString(9));
                list.add(nv);
            }
            return list;
        } catch (Exception e) {
            e.printStackTrace();
            return null;
        }
    }

    @Override
    public int Them(NhanVien nv) {
        sql = "INSERT INTO tblnhanvien(hoten,gioitinh,diachi,Dienthoai,email,Luong,"
                + "Chucvu,Trangthai)value (?,?,?,?,?,?,?,?)";
        try {
            con = DBconnect1.getConnection();
            ps = con.prepareStatement(sql);
            ps.setObject(1, nv.getMaNV());
            ps.setObject(2, nv.getHoTen());
            ps.setObject(3, nv.getGioiTinh());
            ps.setObject(4, nv.getDiaChi());
            ps.setObject(5, nv.getSDT());
            ps.setObject(6, nv.getEmail());
            ps.setObject(7, nv.getLuong());
            ps.setObject(8, nv.getChucVu());
            ps.setObject(9, nv.getTrangThai());
            return ps.executeUpdate();
        } catch (Exception e) {
            e.printStackTrace();
            return 0;
        }
    }

    @Override
    public int Sua(NhanVien nv, String maNV) {
        sql = "update tblnhanvien set HoTen=?,GioiTinh=?,DiaChi=?,DienThoai=?,"
                + "Email=?,Luong=?,ChucVu=?,TrangThai=? where MaNV=?";
        try {
            con = DBconnect1.getConnection();
            ps = con.prepareStatement(sql);
            ps.setObject(1, maNV);
            ps.setObject(2, nv.getHoTen());
            ps.setObject(3, nv.getGioiTinh());
            ps.setObject(4, nv.getDiaChi());
            ps.setObject(5, nv.getSDT());
            ps.setObject(6, nv.getEmail());
            ps.setObject(7, nv.getLuong());
            ps.setObject(8, nv.getChucVu());
            ps.setObject(9, nv.getTrangThai());
            return ps.executeUpdate();
        } catch (Exception e) {
            e.printStackTrace();
            return 0;
        }
    }

}
